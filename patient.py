...
# =========================
con = sqltor.connect(host="localhost", user="root", password="Kanisql#")
cur = con.cursor(buffered=True)

def setup_db():
    cur.execute("CREATE DATABASE IF NOT EXISTS apptdb")
    cur.execute("USE apptdb")
    ...
    CREATE TABLE IF NOT EXISTS appointments (
        ...
        ON DELETE CASCADE,
        UNIQUE KEY uniq_doctor_date_no (doctor_name, appt_date, appt_no)
    )
    """)
...
def ensure_unique_appt_no(doctor_name, appt_date):
    # Try random 2-digit numbers until unique for that doctor+date
    for _ in range(100):
        num = random.randint(10, 99)
        cur.execute("""
            SELECT 1 FROM appointments WHERE doctor_name=%s AND appt_date=%s AND appt_no=%s""", (doctor_name, appt_date, num))
        if not cur.fetchone():
            return num
    
    # Fallback to sequential if random fails
    cur.execute("""
        SELECT COALESCE(MAX(appt_no), 9) FROM appointments WHERE doctor_name=%s AND appt_date=%s""", (doctor_name, appt_date))
...
def register_patient():
    ...
    cur.execute(
        "INSERT INTO appt (idno, name, age, gender, phone, bg) VALUES (%s, %s, %s, %s, %s, %s)",(idn, name, age, gender, phone, bg))
    con.commit()
...
def book_appointment():
    ...
    chosen = random.choice(candidates)  # (name, dept, room)
    
    # Department-specific waiting days
    days_map = {
        "Cardiologist": 3,
        "Rheumatologist": 5,
        "Psychiatrist": 3,
    ...
    cur.execute("""INSERT INTO appointments(patient_id, doctor_name, department, room, appt_date, appt_no)VALUES (%s, %s, %s, %s, %s, %s)""", (patient_id, chosen[0], chosen[1], chosen[2], appt_date, appt_no))
    con.commit()
...
def doctor_login():
    ...
    name_input = input("\nEnter your doctor name (exact as above, or press Enter to cancel): ").strip()
    if not name_input:
        return
    ...
    # Show this doctor's appointments (upcoming first)
    cur.execute("""
